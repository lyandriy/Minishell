...
/*   Updated: 2023/08/23 19:27:22 by lyandriy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fill_argv(t_cmd *new_cmd, char *path_with_command)
{
	...
	if (ft_strchr(path_with_command, '/'))
...
void	ptr_path(t_cmd *new_cmd, char **separate_path, int *count)
{
...
void	ft_path(t_shell *shell, t_cmd *new_cmd)
{
	...
	count = 0;
	if (access(new_cmd->argv[0], F_OK) == -1
		&& ft_strchr(new_cmd->argv[0], '/'))
		return ;
	ptr = should_get_path(new_cmd, shell, &count, &separate_path);
	if (ptr)
		ptr_path(new_cmd, separate_path, &count);
	else
	{
		if (access(new_cmd->argv[0], F_OK) == 0)
			new_cmd->filepath = ft_strdup(new_cmd->argv[0]);
	}
}
